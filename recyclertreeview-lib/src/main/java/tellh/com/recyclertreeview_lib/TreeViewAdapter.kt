...
class TreeViewAdapter(nodes: List<TreeNode<*>>?, viewBinders: List<TreeViewBinder<*>>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        for (viewBinder in viewBinders) {
            if (viewBinder.layoutId == viewType) return viewBinder.provideViewHolder(v)
        }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener(View.OnClickListener {
            ...
            holder.itemView.tag = System.currentTimeMillis()
            if (onTreeNodeListener != null && onTreeNodeListener!!.onClick(selectedNode, holder)) return@OnClickListener
